...
class OrderApi {
  ...
  async confirmPayment({ token, clientId }, orderID) {
    try {
      const res = await axios.post(
        `${BASE_URL}/order/confirm-payment/${orderID}`,
        {},
        {
          headers: configHeader({ token, clientId }),
        }
  ...
  async completeOrder({ token, clientId }, orderID) {
    try {
      ...
      const res = await axios.post(
        `${BASE_URL}/order/completeOrder/${orderID}`,
        {},
        {
          headers: configHeader({ token, clientId }),
        }
  ...
  async getOrderDetail({ token, clientId }, orderID) {
    try {
      const res = await axios.get(
        `${BASE_URL}/order/details?orderId=${orderID}`,
        {
          headers: configHeader({ token, clientId }),
        }
      );
  ...
  async cancelOrder({ token, clientId }, orderID) {
    try {
      const res = await axios.post(`${BASE_URL}/order/cancle/${orderID}`, {
        headers: configHeader({ token, clientId }),
      });
      return res.data;
    } catch (error) {
